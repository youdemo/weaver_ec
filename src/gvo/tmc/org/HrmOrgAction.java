...
public class HrmOrgAction {
	...
	public ReturnInfo operResource(HrmResourceBean hrb){
		...
		String password="";
		String sql = "";
		sql = "select id,password from hrmresource where workcode='"+workcode+"' ";
		rs.executeSql(sql);
		int id = 0;
		if(rs.next()){
			id = rs.getInt("id");
		...
		if(id < 1){
			...
			if(hrb.getBelongIdOrCode()==1){
				//mapStr.put("password","");
			}else{
				if("".equals(hrb.getPassword())){
					mapStr.put("password","753DE9EC72646DAF79C369C150DAA27C");
				}else{
			...
			//所有上级
			mapStr.put("managerstr",hrb.getBankid1());
			int s = 0;
...
//			mapStr.put("workcode", workcode);
		...
		updateMap = hrb.getCusMap2();
		mainMap = new HashMap<String,String>();			
		mainMap.put("id", ""+id);
		mainMap.put("scope", "HrmCustomFieldByInfoType");
		mainMap.put("scopeid", "1");
		message = message + updateCustom(tableName,mainMap,updateMap);	
		ri.setRemark(message);	
